...
The sporadic merges also gives a composition model but where the choices to make a composition of are very limited (the one we have since release or a dev. one) leading to collaboration problems.

The branching pattern we propose on the module level are to have each group to develop their module in  a developement branch and when it's \emph{reasonably} stable merge that to a
